...
var minimumSemesters = function(N, relations) {
    ...
    // add courses to graph and track how many time has it appeared as as course that requires prereq
    for(let [v, e] of relations) {
        if(!graph[v] ) graph[v] = [[], 0];
        if(!graph[e] ) graph[e] = [[], 0];
        graph[v][0].push(e)
        graph[e][1]++;
    }
    // check if there are courses that require prereq
    for (let p in graph){
        if(graph[p][1]==0) {
            count++;
            q.push(p)};
    }
    // if queue is empty, it means all courses required prereq = invalid
    while(q.length){
        let len = q.length;
        while(len--){
            let cur = q.shift();
            for(let c of graph[cur][0]){
                graph[c][1]--;
                if(graph[c][1]==0){
                    q.push(c);
                    count++
